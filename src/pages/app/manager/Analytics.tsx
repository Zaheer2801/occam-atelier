...
const ManagerAnalytics = () => {
  ...
  const byClient = useMemo(() => {
    ...
    return Object.entries(m).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([client, count]) => ({ client, count }));
  }, [apps]);
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={byClient} layout="vertical">
              ...
              <YAxis type="category" dataKey="client" stroke="hsl(var(--muted-foreground))" fontSize={11} width={90} />
              <Tooltip contentStyle={{ background: "hsl(var(--popover))", border: "1px solid hsl(var(--border))", borderRadius: 8 }} />
